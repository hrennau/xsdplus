...
(: eval @ctxt 
        ...
        (: isConditional :)
        let $isConditional := exists($n/@alt)
...
(:~ 
 ...
 :)
declare function f:seatx_functions_valueMap($resources as element(z:prolog))
...
(:~
 ...
 :) 
declare function f:seatx_functions($resources as element(z:prolog))
...
declare function f:seatx_expressionValue($attValue as xs:string?, $atomize as xs:boolean?)
    ...
    else if (matches(replace($attValue, '^.+/', ''), '^[&quot;&apos;]')) then $attValue    
