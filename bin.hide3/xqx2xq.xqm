...
(:~
 ...
 : @return XQuery code
 :) 
declare function f:xqx2_xq($xqx as element(z:xquery))
        as item()* {
    let $codeWrapped :=
    ...
    <_>{
        ...
        text {'&#xA;'},   
        f:xqx2_xqRC($xqx/descendant::z:set-context[1], 0)
    }</_>
    return
        $codeWrapped/node()
};
...
(:~
 : Recursive helper function of `xqx2_xq`.
 :
 : @param n the node to be processed
 : @return the XQuery code represented by this node
 :) 
declare function f:xqx2_xqRC($n as node(), $level as xs:integer)
        as item()* {
    typeswitch($n)        
    case document-node() return
        document {
            for $c in $n/node() return f:xqx2_xqRC($c, $level)
        }
    ...
    case element(z:choice) return
        ...
        return (
            ...
            let $prefix := if ($pos eq 1) then () else 'else '
            let $line := concat($indent, $prefix, 'if (', $branch/@expr,  ') then')
            let $branchContent := $branch/* ! f:xqx2_xqRC(., $level + 1)
            return (
                text {$line || '&#xA;'},
                $branchContent,
                text {'&#xA;'}
        ...
        return
            if (not($n/z:then/*)) then
                if (not($n/z:else/*)) then
                    ...
                    for $c in $n/z:else/* return f:xqx2_xqRC(., $level)
                )
            else (
                $ifCondThen,
                for $c in $n/z:then/* return f:xqx2_xqRC(., $level + 1),
                concat($indent, 'else'),
                for $c in $n/z:then/* return f:xqx2_xqRC(., $level + 1)
            )
            
    case element(z:attributes) return
        for $c in $n/* return f:xqx2_xqRC($c, $level)
    ...
    case element(z:attribute) return
        ...
        return
            if ($expr) then concat($attributeName, $expr, '}')
                ...
                $attributeName,
                for $c in $n/* return f:xqx2_xqRC(., $level +1),
                concat($indent, '}')                
            )
        ...
        return
            element {node-name($n)} {
                if ($expr) then concat('{', $expr, '}')
                    ...
                    '{',
                    for $c in $n/* return f:xqx2_xqRC($c, $level + 1),
                    '}'
                ) 
            }
    default return $n
